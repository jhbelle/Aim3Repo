...
RH = nc_open("T:/eohprojs/CDC_climatechange/Jess/Dissertation/Paper3_Data/rhum.2m.2003.nc")
...
PBLh = nc_open("T:/eohprojs/CDC_climatechange/Jess/Dissertation/Paper3_Data/hpbl.2003.nc")
hPBL = ncvar_get(PBLh, "hpbl")

# Read in matching between MAIAC and NARR grids
MAIACtoNARR = read.csv("T:/eohprojs/CDC_climatechange/Jess/Dissertation/Paper3_StudyDefs/FinGrid/MAIACto_NEI2008_NARR_DistRds.csv", stringsAsFactors = F)[,c("Input_FID", "Count_", "NEAR_DIST", "lat", "lon")]
...
for (i in Startday:Endday){
  ...
  CombDat = read.csv(CombFile, stringsAsFactors = F)
