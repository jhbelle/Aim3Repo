...
data One;
set data.otitis; /*[insert your health dataset]*/
where mdy(1,1,2003)<=admitdate<=mdy(12,31,2005); /*[insert your time period]*/
keep admitdate zip5 ear_any;
proc contents;
run;
...
data four;
set three;
if ear_any ge 1 and i=0 then delete;
run; 
...
data sixA sixB;
set five;
if ear_any ge 1 then output sixA;
else output sixB;
run;
...
*drop AS_WHZ ear_any EDALL RESP_WHZ; /*drop all health variables because your new health outcome variable is called "outcome")*/
run;
...
CloudGapFillIQR = CloudGapFill/6.794054;
NoCldGapFillIQR = NoCldGapFill/6.935553;
UnGapFillIQR = UnGapFill/6.979521;
