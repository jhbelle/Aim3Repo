pro procMaiac, FinGridJoined, yrint, startday, endday, fileloc, outloc, maiacString, LocTAFlag, LocTStamp
  ...
  Column=Dat.(WHERE(DatHead EQ 'Avg_col'))
  Row=Dat.(WHERE(DatHead EQ 'Avg_row'))
	...
	; Open file to append to later
  	...
  	PRINTF, 1, "InputFID, row, col, lat, lon, Year, Date, Timestamp, Overpass, AOD47, AOD55, QA"
        FOR J = 0, N_ELEMENTS(AODQA)-1 DO BEGIN
          PRINTF, 1, Dat.(WHERE(DatHead EQ "Input_FID"))[J], Dat.(WHERE(DatHead EQ "Avg_col"))[J], Dat.(WHERE(DatHead EQ "Avg_row"))[J], Dat.(WHERE(DatHead EQ "Avg_lat"))[J], Dat.(WHERE(DatHead EQ "Avg_lon"))[J], yrint, day, TStamp, TerraAquaFlag, AOD47[J], AOD55[J], AODQA[J], FORMAT='(I10, 4(", ", D), 2(", ", I5), 2(", ", A15), 3(", ", I15))'
        ENDFOR
        CLOSE, 1
      ENDIF
